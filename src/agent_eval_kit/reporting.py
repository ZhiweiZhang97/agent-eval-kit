...
def write_markdown(path: str | Path, results: list[EvalResult]) -> None:
    ...
    for r in results:
        ...
        lines.append(f"| {r.id} | {status} | {r.latency_ms:.1f} | {failed} |")
...
def write_html(path: str | Path, results: list[EvalResult]) -> None:
    ...
    for r in results:
        ...
        rows.append(
            ...
            f"<pre>{html.escape(r.response)}</pre></details></td>"
            "</tr>"
        )
    doc = f"""<!doctype html>
<html lang="en">
...
<table>
<thead><tr><th>Case</th><th>Status</th><th>Latency</th><th>Checks</th><th>Output</th></tr></thead>
...
def write_junit(path: str | Path, results: list[EvalResult]) -> None:
    ...
    for r in results:
        ...
        if not r.passed:
            ...
            message = "; ".join(failed) or r.error or "Evaluation failed"
